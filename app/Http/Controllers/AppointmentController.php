...
//        $doctors = Doctor::select('doctorId', 'firstName', 'lastName')->get();
        $doctors = WorkingHour::select('fkdoctorId','doctorId','firstName','lastName')
...
//DB::raw("CASE WHEN gender = 1 THEN 'Male' WHEN gender = 2 THEN 'Female' END  AS gender"),
//DB::raw("CASE WHEN status = 0 THEN 'Deleted' WHEN status = 1 THEN 'Active' WHEN status = 2 THEN 'Inactive' END  AS status")

    public function showAppointment() {
        $appointmentInfo = Appointment::select(DB::raw("concat(`patient`.`firstName`, ' ' , `patient`.`lastName`) as patientname"), DB::raw("concat(`doctor`.`firstName`, ' ' , `doctor`.`lastName`) as doctorname"), 'fkdoctorId', 'fkpatientId', 'appointment.age', DB::raw("CASE WHEN patient.gender = 1 THEN 'Male' WHEN patient.gender = 2 THEN 'Female' END  AS gender"), 'appointment.email','appointment.phone','appointment.address', DB::raw("CASE WHEN doctor.status = 0 THEN 'Deleted' WHEN doctor.status = 1 THEN 'Active' WHEN doctor.status = 2 THEN 'Inactive' END  AS status"), 'appointment.appointmentId')
            ->leftjoin('doctor','fkdoctorId','doctorId')
            ->leftjoin('patient','fkpatientId','patientId')
            ->get();
